...
import { listSalesRecord, getTotalBalanceSum, getSalesRecord, deleteSalesRecord, deleteAllSalesRecords } from "../../services/SaleService";
...
const Sales = () => {
    ...
    const handleEditRecord = (row) => {
        getSalesRecord(row.id).then((response) => {
    ...
    const handleDeleteRecord = () => {
        if (isBulkDelete) {
            ...
            deleteSalesRecord(deleteRecord).then(() => {
                setRows((prevRows) => prevRows.filter((row) => row.id !== deleteRecord));
                getTotalBalanceSum().then((response) => {
                    setTotalBalanceSum(response.data);
                }).catch(error => {
                    console.error("Error fetching total balance sum: ", error);
    ...
    return (
        ...
        <Box m="20px">
            ...
            <Box display="flex" justifyContent="space-between" p={2}>
                <Typography variant="h5">Total Balance Sum: </Typography>
                <Typography variant="h5" sx={{ fontWeight: "bold"}}>₦{totalBalanceSum}</Typography>
            </Box>
